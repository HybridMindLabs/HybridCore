...
<template>
    <div>
        <div class="flex items-baseline justify-between mb-1.5 gap-2">
            ...
            <span v-if="optional" class="text-zinc-600 text-[10px] font-medium uppercase tracking-wider">Optional</span>
        </div>
        ...
        <input
            ...
            class="w-full bg-zinc-900/60 border text-zinc-100 rounded-xl px-3.5 py-2.5 text-sm placeholder:text-zinc-600 focus:outline-none focus:ring-2 transition"
            :class="error
        ...
        <p v-else-if="hint" class="text-zinc-600 text-xs mt-1.5 leading-relaxed">{{ hint }}</p>
    </div>
</template>
