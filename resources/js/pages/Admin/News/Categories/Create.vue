...
const form = useForm({
    name: '', slug: '', description: '', color: '#3b82f6', icon: 'Newspaper',
    ...
    <AdminLayout title="New Category">
        ...
        <div class="max-w-2xl">
            <div class="rounded-2xl border border-zinc-800/70 bg-[#111113] overflow-hidden">
                ...
                <form class="p-5 flex flex-col gap-4" @submit.prevent="form.post(route('admin.news.categories.store'))">

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label :class="lc">Name</label>
                            <input v-model="form.name" :class="[ic, form.errors.name ? 'border-red-500/50' : '']" placeholder="Gaming News" />
                            <p v-if="form.errors.name" class="text-red-400 text-xs mt-1">{{ form.errors.name }}</p>
                        </div>
                        <div>
                            <label :class="lc">Slug <span class="normal-case text-zinc-700 font-normal">(auto)</span></label>
                            <input v-model="form.slug" :class="[ic, form.errors.slug ? 'border-red-500/50' : '']"
                                placeholder="gaming-news" @input="slugEdited = true" />
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description" :class="ic" rows="2" placeholder="Short description..." />
                    </div>
