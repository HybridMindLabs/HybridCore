...
import { Plus, Pencil, Trash2, Eye, Pin, Star, Newspaper, ChevronDown, CheckCircle2, Archive, Search, X } from '@lucide/vue';
...
interface Filters { search: string; category_id: number | null; status: string }

const props = defineProps<{ articles: Pagination; categories: Category[]; filters: Filters }>();
...
function applyFilters() {
    ...
    router.get(route('admin.news.articles.index'), {
...
function pageHref(p: number) {
    return route('admin.news.articles.index', {
        page: p,
        search: search.value || undefined,
        category_id: categoryFilter.value || undefined,
        status: statusFilter.value || undefined,
    ...
    <AdminLayout title="News Articles">
        ...
        <div class="flex items-center gap-2 mb-4 flex-wrap">
            <div class="relative flex-1 min-w-[200px] max-w-sm">
                <Search :size="13" :stroke-width="1.8" class="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600 pointer-events-none" />
                <input
                    v-model="search"
                    type="text"
                    placeholder="Search by title…"
                    class="w-full bg-[#111113] border border-zinc-800/70 text-zinc-100 rounded-xl pl-9 pr-8 py-2 text-[13px] placeholder:text-zinc-600 focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/10 transition"
                />
            ...
            <select v-model="categoryFilter" class="bg-[#111113] border border-zinc-800/70 rounded-xl px-3 py-2 text-[13px] text-zinc-100 focus:outline-none focus:border-blue-500/50">
                <option value="">All categories</option>
                <option v-for="c in categories" :key="c.id" :value="c.id">{{ c.name }}</option>
            </select>
            <select v-model="statusFilter" class="bg-[#111113] border border-zinc-800/70 rounded-xl px-3 py-2 text-[13px] text-zinc-100 focus:outline-none focus:border-blue-500/50">
        ...
        <div class="rounded-xl border border-zinc-800/70 bg-[#111113] overflow-hidden transition-opacity" :class="searching ? 'opacity-50' : ''">
            ...
            <EmptyState
                ...
                :icon="Newspaper"
                :title="search || categoryFilter || statusFilter ? 'No matches' : 'No articles yet'"
                :description="search || categoryFilter || statusFilter ? 'Try a different search term or filter.' : 'Write your first article to get started.'"
            >
                <template v-if="!search && !categoryFilter && !statusFilter" #action>
                    <Link :href="route('admin.news.articles.create')" class="text-blue-400 text-[13px] font-semibold hover:underline">Write one</Link>
                </template>
            </EmptyState>

            <div v-for="(a, i) in articles.data" :key="a.id"
                class="hc-hero-in grid grid-cols-1 lg:grid-cols-[28px_minmax(0,1fr)_120px_100px_80px_70px_80px] items-center gap-3 px-5 py-3.5 border-b border-zinc-800/50 last:border-0 hover:bg-white/[0.02]"
