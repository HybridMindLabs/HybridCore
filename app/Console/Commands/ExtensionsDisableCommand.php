...
{
    protected $signature = 'hybridcore:extensions:disable {slug : Extension slug (e.g. hybridcore/announcements)}';

    protected $description = 'Disable an extension by slug and queue an asset rebuild';
    ...
    public function handle(ExtensionManager $manager): int
    {
        ...
        if (! $extension->enabled) {
            $this->info("{$extension->name} is already disabled.");
        ...
        $this->info("{$extension->name} disabled. Asset rebuild queued.");
